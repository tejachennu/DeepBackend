...
class CampaignRegistrationField {
    static async create(data) {
        const [result] = await db.execute(
            `INSERT INTO campaign_registration_fields 
             (CampaignId, FieldName, FieldLabel, FieldType, Options, IsRequired, DisplayOrder, CreatedBy)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                data.campaignId,
                data.fieldName,
    ...
    static async findByCampaign(campaignId) {
        const [rows] = await db.execute(
            `SELECT * FROM campaign_registration_fields 
             WHERE CampaignId = ? AND IsDeleted = FALSE AND IsActive = TRUE
    ...
    static async findById(id) {
        const [rows] = await db.execute(
            `SELECT * FROM campaign_registration_fields WHERE FieldId = ? AND IsDeleted = FALSE`,
            [id]
        );
    ...
    static async update(id, data, userId) {
        ...
        const [result] = await db.execute(
            `UPDATE campaign_registration_fields SET ${fields.join(', ')} WHERE FieldId = ? AND IsDeleted = FALSE`,
            values
        );
    ...
    static async delete(id, userId) {
        const [result] = await db.execute(
            `UPDATE campaign_registration_fields SET IsDeleted = TRUE, UpdatedBy = ? WHERE FieldId = ?`,
            [userId, id]
        );
        return result.affectedRows > 0;
    }
...
module.exports = CampaignRegistrationField;
